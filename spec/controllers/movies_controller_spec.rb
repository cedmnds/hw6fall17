...
describe MoviesController do
  describe 'searching TMDb' do
   it 'should call the model method that performs TMDb search' do
    ...
    it 'should select the Search Results template for rendering if valid search' do
    ...
    it 'should show an error message if user entered an invalid search' do
      fake_results = [double('Movie1'), double('Movie2')]
      allow(Movie).to receive(:find_in_tmdb).and_return(fake_results)
      post :search_tmdb, {:search_terms => ''}
      expect(flash[:warning]).to eq('Error. Invalid search.')
    end
    
    it 'should flash message to user if no results were found in the search' do
      fake_results = []
      allow(Movie).to receive(:find_in_tmdb).and_return(fake_results)
      post :search_tmdb, {:search_terms => 'Test'}
      expect(flash[:warning]).to eq('No results were found on TMDb')
    end
  end
  describe 'adding a list of checked movies from search results' do
    context 'when no boxes are checked' do
      it 'should set a flash message alerting that no options were selected' do
        ...
        expect(flash[:warning]).to eq('Error. No movies selected')
      end
    end
